...
@section('content_header')
    <div class="card">
        <div class="card-body">
            <h1>SE MOSTRARAN TODOS LOS DESTINOS</h1>
        </div>
    </div>
@stop
    ...
    <div class="card">
        <div class="card-body">
            @can('admin.destinos.create')
                <a href="{{ route('admin.destinos.create') }}" class="btn btn-primary float-right mt-2 mr-3">NUEVO DESTINO</a>
            @endcan
            <br>
            <br>
            <br>
            <table class="table table-striper">
                ...
                <tbody>
                    @foreach ($destinos as $des)
                        <tr>
                            <td>{{ $des->ID_Destino }}</td>
                            <td>{{ $des->desCodigo }}</td>
                            <td>{{ $des->desNombre }}</td>
                            <td>{{ $des->desDescripcion }}</td>
                            <td>{{ $des->desDireccion }}</td>
                            <td>
                                @php
                                    $estados = [
                                        1 => 'ACTIVO',
                            ...
                            <td width="10px">
                                <a class="btn btn-primary" href="{{ route('admin.destinos.edit', $des) }}">Editar</a>
                            </td>
                            @endcan
                            @can('admin.destinos.destroy')
                            <td width="10px">
                                <form class="formEliminar" action="{{ route('admin.destinos.destroy', $des) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
